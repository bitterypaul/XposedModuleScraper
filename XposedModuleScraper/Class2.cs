...
namespace XposedModuleScraper
{
    class Program
    {
        ...
        static void SetDetails()
        {
            ...
            {
                allModules.Add(new ModuleDetails());
            }
        ...
        public static void DownloadUrlsInParallel(Uri[] urls)
        {
            var tasks = urls
                .Select(url => Task.Factory.StartNew(
                    state =>
                    {
                        using (var client = new System.Net.WebClient())
                        {
                            ...
                            string urii = u.OriginalString;
                            urii = urii.Substring(UrlTemplate.Length);
                            int listingPage = Convert.ToInt32(urii);
                            return new KeyValuePair<int, string>(listingPage, result);
                        }
                    }, url)
                )
                .ToArray();
            ...
            foreach (var t in tasks)
            {
                ...
                while ((temp1.Contains("<a href=\"/module/")))
                {
                    ...
                    if (moduleUrl != "de.robv.android.xposed.installer")
                    {
                        ...
                        allModules[i + (t.Result.Key)] = moduleDetails;
                        i++;
                    }
                }
        ...
        static void Main(string[] args)
        {
            ...
            foreach (ModuleDetails mm in allModules)
            {
                Console.WriteLine(mm.title);
            }
            Console.WriteLine();
